...
func main() {
	...
	// Check which points overlap and find the claim that doesn't overlap
	...
	for claimID := range claimsMap {
		overlapFound := false
		for i := 0; i < len(claimsMap[claimID]); i++ {
			if overlapMap[claimsMap[claimID][i]] > 1 {
				overlapFound = true
			}
		}
		if overlapFound == false {
			fmt.Println("The ID of the claim that does not overlap is:", claimID)
		}
	}
}
